...
mod nelder_mead {
    ...
    impl minimizer::Minimizer for NelderMead {
            ...
            F: Fn(ArrayView1<f64>) -> f64,
        {
            ...
            loop {
                if maxiter.map_or(false, |v| v <= iterations)
                    || maxfun.map_or(false, |v| v <= func.num)
                {
                    break;
                }
...
mod tests {
    ...
    fn simplex() {
        ...
        let minimizer = nelder_mead::NelderMeadBuilder::default()
            .xtol(1e-8f64)
            .ftol(1e-8f64)
            .build()
            .unwrap();
    ...
    fn gradient() {
        ...
        let res = utils::approx_fprime(x.view(), function, eps_ar.view());
